...
def validate(value):
    ...
    if (
        type(value.get("ticket_id")) is not int
        or not 0 < value["ticket_id"] < 1_000_000_000
        or value.get("project") not in PROJECTS
    ):
    ...
    if "release_id" in value and (
        ...
        or not re.fullmatch(r"[a-f0-9-]{36}|[a-f0-9]{64}", value["release_id"])
    ):
...
def fingerprint(task):
    ...
    body["payload"] = {
        key: payload.get(key)
        for key in ("description", "context_refs", "amendments", "lineage", "originating_task_id")
    }
