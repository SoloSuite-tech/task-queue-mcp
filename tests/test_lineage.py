...
def test_lineage_is_typed_and_inherited_by_authorized_followup(tmp_path):
    ...
    assert first["ok"]
    child = submit_task_handler(
        source_agent="builder",
        target_agent="deployer",
        task_type="deploy",
        summary="Release",
        description="Deploy selected artifact",
        originating_task_id=first["task_id"],
        queue_dir=str(tmp_path),
    )
    assert get_task_handler(child["task_id"], str(tmp_path))["payload"]["lineage"] == {
        "ticket_id": 62,
        "project": "schlagbaum",
    }
    refused = submit_task_handler(
        source_agent="builder",
        target_agent="deployer",
        task_type="deploy",
        summary="Release",
        ...
        originating_task_id=first["task_id"],
        queue_dir=str(tmp_path),
        lineage={"ticket_id": 99, "project": "parker"},
    )
...
def test_invalid_lineage_does_not_write(tmp_path):
    for lineage in [
        {"ticket_id": True, "project": "parker"},
        {"ticket_id": 1, "project": "foreign"},
        {"ticket_id": 1, "project": "parker", "authority": "admin"},
    ]:
...
def test_approval_refuses_amendment_between_review_and_approval(tmp_path):
    ...
    assert amend_task_handler(
        tid, "Additional production operation", "planner", queue_dir=str(tmp_path)
    )["ok"]
    refused = set_task_status_handler(
        tid, "approved", "operator", expected_fingerprint=reviewed, queue_dir=str(tmp_path)
    )
    assert refused["ok"] is False
    current = get_task_handler(tid, str(tmp_path))
    assert current["status"] == "submitted"
    assert set_task_status_handler(
        tid,
        "approved",
        "operator",
